...
class S3FsFilesSource(BaseFilesSource):
    ...
    def _list(
        ...
        if recursive:
            ...
            ###  Fix for individual buckets : only buckets belonging to the user will be accessible
            import re
            username=user_context.username
            search_pattern = rf"/{re.escape(username)}-bucket"
            user_accessible_prefixes = []
            # Filter prefixes (bucket subdirectories) and dispaly only those which belong to the user
            for item in res:
                 if re.search(search_pattern,item['Key']):
                     user_accessible_prefixes.append(item)
            res = user_accessible_prefixes
